...
def addForce(pts, pts_spd, root, root_locs, pos_data, gravity):
    for i, p in enumerate( pts, 0 ):
        ...
        if bpy.context.window_manager.spring_bone_keep_is_spring:
            spd = pts_spd[i]
        
            diff = getDiffPosition( root_locs, i * 3 )
            
            spd += diff * bpy.context.window_manager.spring_bone_extend_factor
            spd += back_force * bpy.context.window_manager.spring_bone_spring_factor
            spd += gravity * bpy.context.window_manager.spring_bone_gravity_factor
            spd *= bpy.context.window_manager.spring_bone_friction_factor
            pts[i] = setTranslationForMatrix( p, p.to_translation() + spd )
        else:
            pts[i] = setTranslationForMatrix( p, p.to_translation() + back_force * bpy.context.window_manager.spring_bone_spring_factor )
...
class vic_spring_bone(bpy.types.Operator):
    ...
    def process(self,context):
        ...
        start_frame = context.window_manager.spring_bone_frame_start
        end_frame = context.window_manager.spring_bone_frame_end
...
class VIC_PT_SPRING_BONE_TOOL(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.operator(vic_spring_bone.bl_idname)        
        col.operator(vic_bones_clear_key.bl_idname)        
        col.prop(context.window_manager, 'spring_bone_frame_start' ) 
        col.prop(context.window_manager, 'spring_bone_frame_end' ) 
        col = layout.column(align=True)
        col.prop(context.window_manager, 'spring_bone_extend_factor' ) 
        col.prop(context.window_manager, 'spring_bone_spring_factor' ) 
        col.prop(context.window_manager, 'spring_bone_gravity_factor' ) 
        col.prop(context.window_manager, 'spring_bone_friction_factor' ) 
        col.prop(context.window_manager, 'spring_bone_keep_is_spring' ) 
       # col = layout.column(align=True)
       # col.prop(context.scene, 'spring_bone_roop_gravity', 'Roop Gravity' ) 
        
#=======================================

bpy.types.WindowManager.spring_bone_extend_factor = bpy.props.FloatProperty(
    name='Extend',
    default=0.0,
    min=0.0,
    max=1.0
...
bpy.types.WindowManager.spring_bone_spring_factor = bpy.props.FloatProperty(
    name='Keep',
    default=.6,
    min=0.0,
    max=1.0
...
bpy.types.WindowManager.spring_bone_gravity_factor = bpy.props.FloatProperty(
    name='Gravity',
    default=0.0,
    min=0.0,
    max=1.0
...
bpy.types.WindowManager.spring_bone_friction_factor = bpy.props.FloatProperty(
    name='Friction',
    default=0.5,
    min=0.0,
    max=1.0
...
bpy.types.WindowManager.spring_bone_keep_is_spring = bpy.props.BoolProperty(
    name='Spring',
    default=True)

bpy.types.WindowManager.spring_bone_frame_start = bpy.props.IntProperty(
    name="Start Frame", description="Start frame of simulation", 
    default=1, step=1, min=1, max=100000)
        
bpy.types.WindowManager.spring_bone_frame_end = bpy.props.IntProperty(
